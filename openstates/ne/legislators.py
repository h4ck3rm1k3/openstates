    ...
    def scrape(self, term, chambers):
        ...
        for district in range(1, 50):
            ...
            try:
                ...
                full_name = page.xpath(
                    '//div[@class="content_header_right"]/a')[0].text.split(' ', 1)[1].strip()
                # This is hacky, are lis always the same?
                address = page.xpath(
                    '//div[@id="sidebar"]/ul[1]/li[3]')[0].text.strip() + '\n'
                address += page.xpath('//div[@id="sidebar"]/ul[1]/li[4]')[0].text.strip() + \
                    '\n'
                address += page.xpath('//div[@id="sidebar"]/ul[1]/li[5]')[0].text.strip()
                phone = page.xpath(
                    '//div[@id="sidebar"]/ul[1]/li[6]')[0].text.split()
                if len(phone) > 2:
                    phone = phone[1] + ' ' + phone[2]
                else:
                    phone = None
                email = page.xpath(
                    '//div[@id="sidebar"]/ul[1]/li[7]/a')[0].text or ''
