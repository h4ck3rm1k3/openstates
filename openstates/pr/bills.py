...
class PRBillScraper(BillScraper):
    ...
    def parse_action(self, chamber, bill, action, action_url, date):
        ...
        # if action.startswith('Referido'):
            ...
            # print comme
        bill.add_action(action_actor, action.replace('.', ''),
                        date, type=atype)
        return atype, action
    ...
    def scrape_bill(self, chamber, session, bill_id, bill_type):
        ...
        if len(co_authors) != 0:
            for co_author in co_authors[1].split(','):
                bill.add_sponsor(
                    'cosponsor', self.clean_name(co_author).strip())
        ...
        for row in action_table[1:]:
            ...
            atype, action = self.parse_action(
                chamber, bill, action, action_url, date)
...
#           bill_id = header[0][0][3].text_content().lstrip().rstrip();
        ...
        for row in table[0].xpath('tr')[::-1]:
            ...
            abstent_td = tds[4].text_content().replace(
                '\n', ' ').replace(' ', '').replace('&nbsp;', '')
            if party != 'Total':
                name_td = self.clean_name(
                    tds[0].text_content().replace('\n', ' ').replace('', '', 1)).strip()
                split_name = name_td.split(',')
                if len(split_name) > 1:
                    name_td = split_name[1].strip() + \
                        ' ' + split_name[0].strip()
