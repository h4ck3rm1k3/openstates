...
class PREventScraper(EventScraper):
    ...
    def scrape(self, chamber, session):
        ...
        self.upper_url = 'http://www.senadopr.us/Lists/Calendario%20Legislativo/DispForm_original.aspx?ID='
    ...
    def scrape_events(self, chamber, session, event_id):
        ...
        if len(rows):
            ...
            # hack so we dont fail on the first id numbers where there are some
            # gaps between the numbers that work and not.
            if event_id > 1700:
                raise ScrapeError(
